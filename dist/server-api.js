...
import { ensureUser, logUsage, logPayment, updatePaymentStatus, getStats, getUsageLogs, getPaymentLogs, } from "./db.js";
import XLSX from "xlsx";
import { calculate } from "./services/hexagram.js";
import { validateInput } from "./utils/validation.js";
import { DateTime } from "luxon";
import { interpretWithGemini } from "./services/interpret.js";
import { createNativePayment, queryOrder, verifyNotifySignature, decryptNotifyData, checkWechatPayConfig, } from "./services/wechat-pay.js";
    ...
    if (url.pathname.startsWith("/api/")) {
        ...
        if (req.method === "POST" && apiPath === "/payment-notify") {
            ...
            req.on("end", () => {
                try {
                    ...
                    const decryptedData = decryptNotifyData(resource.ciphertext, resource.associated_data, resource.nonce);
                    // 处理支付成功
        ...
        if (req.method === "GET" && apiPath === "/admin/data") {
            ...
            try {
                ...
                const usageLogs = getUsageLogs(200);
                const paymentLogs = getPaymentLogs(200);
                const usageByDate = {};
                const paymentAmountByDate = {};
                stats.usageDaily.forEach((d) => (usageByDate[d.date] = d.count));
                stats.revenueDaily.forEach((d) => (paymentAmountByDate[d.date] = d.amount));
                const usageRecords = usageLogs.map((u) => ({
                ...
                const paymentRecords = paymentLogs.map((p) => ({
        ...
        if (req.method === "GET" && apiPath === "/admin/export") {
            ...
            try {
                const usageRecords = getUsageLogs(1000).map((u) => ({
                    时间: new Date(u.createdAt).toLocaleString("zh-CN"),
                    用户: u.userId,
                    问题: u.question,
                    类型: u.action === "followup" ? "后续问题" : "首次解卦",
                }));
                const paymentRecords = getPaymentLogs(1000).map((p) => ({
