...
const elementsWheel = ["木", "火", "土", "金", "水"];
function elementFromIndex(seed, idx) {
    const pos = Math.abs(seed + idx) % elementsWheel.length;
    return elementsWheel[pos];
}
function relationFromElements(day, line) {
    if (day === line)
        return "兄弟";
    if (generateCycle[line] === day)
        return "父母"; // line generates day
    if (overcomeCycle[line] === day)
        return "官鬼"; // line overcomes day
    if (generateCycle[day] === line)
        return "子孙"; // day generates line
    if (overcomeCycle[day] === line)
        return "妻财"; // day overcomes line
    return "兄弟";
}
function lineTypeFromSeed(seed, idx) {
    const bit = (seed >> idx) & 1;
    return bit === 0 ? "yin" : "yang";
}
function isMovingFromSeed(seed, idx) {
    const pair = (seed >> (idx * 2)) & 0b11;
    return pair === 0 || pair === 3;
}
function buildHexagram(dt, pillarsDay, pillars, userLines) {
